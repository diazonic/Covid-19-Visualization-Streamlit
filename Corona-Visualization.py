...
countries_list = list(set([x.split('_')[0] for x in df.columns]))
countries_list.sort()
...
df_n['daily'] = df_n[select+'_cases'].diff()

option = st.multiselect(
    'Which number do you like best?',[select+'_cases',select+'_recovered',
    select+'_deaths','active','daily']
     )


st.line_chart(df_n[option])
st.sidebar.dataframe(df_n[option].tail(10))
